...
const SessionNotes: React.FC<{ sessionId: string }> = ({ sessionId }) => {
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 64px)', background: '#0b1220', position: 'fixed', top: 64, left: 0, right: 0, bottom: 0 }}>
      
      {/* Сайдбар */}
      <div style={{ width: '280px', minWidth: '280px', maxWidth: '280px', background: '#0f172a', borderRight: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '16px', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
          <a href={`#/session/${sessionId}`} style={{ color: '#475569', fontSize: 12, textDecoration: 'none', display: 'block', marginBottom: 8, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
          {visibleNotes.map(n => (
            <div key={n.id}
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: '#e6eef8', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{n.header}</div>
        ...
        {canEdit && (
          <div style={{ padding: 12, borderTop: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            <button onClick={handleStartCreate} style={{ width: '100%', padding: '9px', borderRadius: 8, border: 'none', background: 'linear-gradient(180deg,#10b981,#059669)', color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
              + Новая заметка
            </button>
          </div>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        {editMode && canEdit && (
          <div style={{ flex: 1, padding: 32, display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto' }}>
            <h2 style={{ color: '#e6eef8', margin: 0, fontSize: 20, fontWeight: 700 }}>
