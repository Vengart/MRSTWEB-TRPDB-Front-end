...
const CreateSession: React.FC<CreateSessionProps> = ({
  isGamemaster, form, setForm, onCreate, onCancel,
}) => {
  if (!isGamemaster) {
    return (
      <div style={{
        ...
        <h2 style={{ color: '#e6eef8', fontSize: '20px', fontWeight: 700, margin: '0 0 8px' }}>
          Доступ ограничен
        </h2>
        ...
        <a href="#/account" style={{
          ...
          borderRadius: '999px', color: '#10b981', fontSize: '14px',
    ...
    <div style={{ maxWidth: '640px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#475569', fontSize: '13px', margin: 0 }}>
          Заполни детали — и собирай команду
        </p>
      </div>
      ...
      <div style={{
        ...
        <div style={field}>
          ...
          <input
            style={input} type="text" value={form.title}
            onChange={e => setForm({ ...form, title: e.target.value })}
            onFocus={focusGreen} onBlur={blurGreen}
            placeholder="Рейд: Хранители Песков"
          />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div style={field}>
            ...
            <input
              style={input} type="text" value={form.date}
              onChange={e => setForm({ ...form, date: e.target.value })}
              onFocus={focusGreen} onBlur={blurGreen}
              placeholder="15 апр 2026"
            />
          </div>
          <div style={field}>
            <label style={label}><Clock size={11} style={{ display: 'inline', marginRight: '4px' }} />Длительность</label>
            <input
              style={input} type="text" value={form.duration}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div style={field}>
            ...
            <input
              style={input} type="text" value={form.price}
          ...
          <div style={field}>
            ...
            <input
              style={input} type="number" value={form.capacity} min="1"
              onChange={e => setForm({ ...form, capacity: parseInt(e.target.value, 10) || 6 })}
              onFocus={focusGreen} onBlur={blurGreen}
            />
          </div>
        </div>
        ...
        {/* Обложка */}
        <div style={field}>
          <label style={label}><Image size={11} style={{ display: 'inline', marginRight: '4px' }} />URL обложки (опционально)</label>
          <input
            style={input} type="text" value={form.image}
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button
            onClick={onCreate}
            disabled={!form.title || !form.game}
            style={{
              flex: 2, padding: '11px 16px', borderRadius: '8px', border: 'none',
              background: (!form.title || !form.game)
                ? 'rgba(255,255,255,0.05)'
                : 'linear-gradient(180deg,#10b981,#059669)',
              color: (!form.title || !form.game) ? '#334155' : 'white',
              fontSize: '14px', fontWeight: 600, cursor: (!form.title || !form.game) ? 'not-allowed' : 'pointer',
